...
import json
import os
from utils.MalConv import MalConv
...
def load_model(model_filepath: str) -> (dict, str):
    ...
    if torch.cuda.is_available():
        device = 'cuda'
    model = MalConv(channels=int(full_conf["num_channels"]),
                    window_size=int(full_conf["filter_size"]),
                    stride=int(full_conf["filter_size"]),
                    embd_size=int(full_conf["embed_size"]))
