...
def tokenize(tokenizer, dataset):
    def strip_leading_whitespace(example):
        if example['question'][0].isspace():
            orig_len = len(example['question'])
            example['question'] = str.lstrip(example['question'])
            delta = orig_len - len(example['question'])
            if len(example['answers']['answer_start']) > 0:
                example['answers']['answer_start'][0] -= delta
        return example

    # remove any leading whitespace from the question
    dataset = dataset.map(strip_leading_whitespace, keep_in_memory=True)
    ...
    # max_seq_length = min(tokenizer.model_max_length, 384)  #1024
    ...
    if not hasattr(tokenizer, 'cls_token') or tokenizer.cls_token is None:
    ...
    def prepare_train_features(examples):
        ...
        try:
            ...
            logging.error("Error tokenizing examples")  # TODO remove before flight
            logging.info("Examples which caused error:")
    ...
    tokenized_dataset = dataset.map(
        ...
        batch_size=8,
        batched=True,
        num_proc=1,  # self.thread_count,  parallelization doesn't help this
