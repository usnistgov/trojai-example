...
class CamExtractor():
    ...
    def __init__(self, model, target_layer):
        ...
        self.handle = self.convs[self.target_layer].register_forward_hook(hook_function)
...
class ScoreCam():
    ...
    def generate_cam(self, input_image, target_class=None):
        ...
        for i in range(len(target)):
            ...
            saliency_map = F.interpolate(saliency_map, size=(input_shape[2], input_shape[3]), mode='bilinear', align_corners=False)
        ...
        cam = np.uint8(Image.fromarray(cam).resize((input_shape[2],
                       input_shape[3]), Image.ANTIALIAS))/255
        return cam
