...
from utils.NeuralNetworkRegressor import NeuralNetworkRegressor
...
class Detector(AbstractDetector):
    def __init__(self, metaparameter_filepath, learned_parameters_dirpath):
        ...
        """
        with open(metaparameter_filepath, 'r') as fin:
            metaparameters = json.load(fin)
    ...
    def automatic_configure(self, models_dirpath: str):
        ...
        for random_seed in np.random.randint(1000, 9999, 10):
            self.weight_table_params["random_seed"] = int(random_seed)
    ...
    def inference_on_example_data(self, model, examples_dirpath):
        ...
        """
        examples_dirpath = pathlib.Path(examples_dirpath)

        example_diffs = []
        for example_path in examples_dirpath.glob("*.json"):
            if not example_path.is_file():
                continue

            # Get the diff
            with example_path.open('r') as fin:
                example_data = json.load(fin)
            diff = base64.b64decode(example_data['diff']).decode('utf-8')
            example_diffs.append((example_data, diff))

        example_embeddings = []
        with model.get_preprocessor("/projects/wg-trojai/llms/CodeLlama-7b-Instruct-hf") as prepper:
            # Note, the preprocessor is the same for all models.  It's
            # projecting the final hidden state of all tokens from a CodeLlama
            # model inference on the tokenized text using a deterministically
            # (via numpy.random.seed) generated random projection matrix.  
            for data, diff in example_diffs:
        ...
        real_labels = []
        pred_labels = []
        for orig_data, embedding in example_embeddings:
            percent_kept = model.predict(embedding)
            if percent_kept > model.threshold:
                real_labels.append(orig_data['blamed_lines'] * 2 >= orig_data['added_lines'])
                pred_labels.append(percent_kept >= model.threshold)

        test_acc = accuracy_score(real_labels, pred_labels)
        print("Model accuracy on example data {}: {}".format(examples_dirpath, test_acc))
