...
class DQNAgent:
    ...
    def __init__(
        ...
        # Categorical DQN parameters
        self.v_min = v_min  # 0
        self.v_max = v_max  # 200
        self.atom_size = atom_size  # 51
        self.support = torch.linspace(
            self.v_min, self.v_max, self.atom_size
        ).to(self.device)  # [0,4,8,...,200]
        ...
        if dqn_savepath:
            with open(dqn_savepath, 'rb') as f:
                record_dict = pickle.load(f)
            self.dqn.load_state_dict(record_dict['dqn_state_dict'])
            print('load from', dqn_savepath)
