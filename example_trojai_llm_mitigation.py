...
def prepare_mitigation(config_json, args):
...
def run_mitigate_mode(args):
    ...
    model, tokenizer = prepare_model_and_tokenizer(args.model, config_json['model_parameters'], args.device)
    peft_config = prepare_peft(config_json['lora_parameters'])
    dataset = prepare_dataset(args.dataset_dirpath)
    print("Finished prepping dataset")
    print_gpu_utilization()
    collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=False)

    mitigation = prepare_mitigation(config_json, args)
