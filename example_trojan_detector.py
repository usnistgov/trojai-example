...
def example_trojan_detector(
    ...
    logging.info(
        "round_training_dataset_dirpath = {}".format(round_training_dataset_dirpath)
    )
    ...
    fns = [
        ...
        if fn.endswith(example_img_format)
    ]
    fns.sort()  # ensure file ordering
    if len(fns) > 5:
        fns = fns[0:5]  # limit to 5 images
    ...
    for fn in fns:
        ...
        logging.info(
            "example img filepath = {}, logits = {}, pred: = {}".format(
                fn, logits, pred
            )
        )
...
def configure(output_parameters_dirpath, configure_models_dirpath, parameter3):
    logging.info("Using parameter3 = {}".format(str(parameter3)))

    logging.info(
        "Configuring detector parameters with models from " + configure_models_dirpath
    )
    ...
    with open(
        os.path.join(output_parameters_dirpath, "dict.json"), mode="w", encoding="utf-8"
    ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--parameter2", type=float, help="An example tunable parameter."
    )
    ...
    if not args.configure_mode:
        if (
            ...
            and args.parameter1 is not None
            and args.parameter2 is not None
        ):

            logging.info("Calling the trojan detector")
            example_trojan_detector(
                args.model_filepath,
