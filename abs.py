...
def check_values(images, labels, model, children, target_layers):
    ...
    for layer_i in range(0, len(children) - 1):
        ...
        max_val = -np.inf
        for i in range(max(len(images)//batch_size,1)):
...
def sample_neuron(images, labels, model, children, target_layers, model_type, mvs):
    ...
    for layer_i in sample_layers:
        ...
        _batch_size = min(batch_size,n_images)
       
        for input_i in range(n_images//_batch_size):
            batch_data = torch.FloatTensor(images[_batch_size*input_i:_batch_size*(input_i+1)])
            batch_data = batch_data.cuda()
            inner_outputs = temp_model1(batch_data).cpu().detach().numpy()
            if channel_last:
                n_neurons = inner_outputs.shape[-1]
            ...
            for nt in range(nbatches):
                ...
                for neuron in range(sample_batch_size):
                    ...
                    for i,v in enumerate(vs):
                        if len(inner_outputs.shape) == 4:
                            if channel_last:
                                h_t[i*_batch_size:(i+1)*_batch_size,:,:,neuron+nt*sample_batch_size] = v
                            else:
                                h_t[i*_batch_size:(i+1)*_batch_size,neuron+nt*sample_batch_size,:,:] = v
                        else:
                            h_t[i*_batch_size:(i+1)*_batch_size, neuron+nt*sample_batch_size] = v
                ...
                fps = temp_model2(f_h_t_t).cpu().detach().numpy() #logits
                # if Print_Level > 1:
                #     print(nt, n_neurons, 'inner_outputs', inner_outputs.shape, 'f_h_t', f_h_t.shape, 'fps', fps.shape)


                for neuron in range(sample_batch_size):
                    tps = fps[neuron*n_samples*_batch_size:(neuron+1)*n_samples*_batch_size]
                    for img_i in range(_batch_size):
                        img_name = (labels[img_i + _batch_size*input_i], img_i + _batch_size*input_i)
                        ps_key= (img_name, '{0}_{1}'.format(children[layer_i].__class__.__name__, layer_i), neuron+nt*sample_batch_size)
                        idx = [ img_i +_batch_size*_ for _ in range(n_samples)]
                        ps = [tps[ img_i +_batch_size*_] for _ in range(n_samples)]
                        ps = np.asarray(ps)
                        ps = ps.T
...
def find_min_max(model_name, all_ps, sample_layers, cut_val=20, top_k = 10):
    ...
    for k in sorted(all_ps.keys()):
        ...
        for l in range(num_classes):
            # amax among points beyond the max_val  minus  amin among points below the max_val
    ...
    #key = (img_id, layer_description, neuron_id)
    neuron_ks = []
    imgs = []
    ...
    #key of neuron_ks is (layer_description, neuron_id)
    min_ps = {}
    min_vals = []
    n_imgs = len(imgs)
    for k in neuron_ks:
        ...
        for img in sorted(imgs):
            ...
            if not ( l in vdict.keys() ):
                ...
                vdict[l].append(v)
        ml = max(set(ls), key=ls.count)  #the major label
    ...
    keys = min_ps.keys()
    keys = []
    #threshold that max affact 0.9 images making them misclassified to the target label
    ...
    #collect the [top_k sampled neurons from early layers, top_k sampled neurons from the last layer]
    ...
    for i in range(len(sorted_key)):
        ...
        # if True:
            ...
            if min_ps[k][2] > max_sampling_val:
                max_sampling_val = min_ps[k][2]
            if Print_Level > 0:
                print('min max val across images', 'k', k, 'label', min_ps[k][0], 'ct', min_ps[k][1], 'value', min_ps[k][2])
                if Print_Level > 1:
                    print(min_ps[k][3])
            allns += 1
            neuron_dict[model_name].append( (layer, neuron, min_ps[k][0]) )
